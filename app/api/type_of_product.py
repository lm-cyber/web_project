...
async def add_type_of_product(type_of_product: NewTypeOfProductModel, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
    return await type_of_product_crud.create(session, type_of_product)


@type_of_product_router.delete("/type_of_product/{id}")
async def delete_type_of_product(id: int, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
...
async def update_type_of_product(type_of_product: TypeOfProductModel, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
