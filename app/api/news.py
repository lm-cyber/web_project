...
async def add_news(news: NewNewsModel, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
    return await news_crud.create(session, news)


@news_router.delete("/news/{id}")
async def delete_news(id: int, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
    await news_crud.delete_news(session, id)


@news_router.put("/news/", response_model=NewsAdded)
async def update_news(news: NewsAdded, session: AsyncSession = Depends(get_async_session), user: User = Depends(current_superuser_user)):
