...
async def delete_image(id: int, session: Session = Depends(get_session)):
    return await news_image_crud.delete(session, id)


@news_image_router.put("/news_image/")
async def update_image(id: int, file_bytes: bytes = File(), session: Session = Depends(get_session)):
