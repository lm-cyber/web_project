...
async def get_product(id: int, session: Session = Depends(get_async_session)):
    return await product_crud.get(session, id)

@product_router.get("/product/", response_model=list[ProductModel])
async def get_products(session: Session = Depends(get_async_session)):
    return await product_crud.get_all(session)


@product_router.post("/product/", response_model=ProductModel)
async def add_product(product: NewProductModel, session: Session = Depends(get_async_session)):
...
async def update_product(product: ProductModel, session: Session = Depends(get_async_session)):
